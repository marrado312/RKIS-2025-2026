...
namespace TodoList
{
	public class FileManager : IDataStorage
	{
		...
		public void SaveProfiles(IEnumerable<Profile> profiles)
		{
			...
			{
				throw new StorageException("Ошибка доступа к файлу при сохранении", ex);
			}
			catch (Exception ex)
			{
				throw new StorageException("Критическая ошибка сохранения", ex);
			}
		}
		...
		public IEnumerable<Profile> LoadProfiles()
		{
			...
			{
				throw new StorageException("Ошибка при чтении или расшифровке данных", ex);
			}
		}
		...
		public void SaveTodos(Guid userId, IEnumerable<TodoItem> todos)
		{
			...
			{
				throw new StorageException($"Ошибка сохранения задач для {userId}", ex);
			}
		}
		...
		public IEnumerable<TodoItem> LoadTodos(Guid userId)
		{
			...
			{
				throw new StorageException($"Ошибка загрузки задач для {userId}", ex);
			}
		}
		...
		{
			SaveProfiles(new List<Profile> { profile });
		}

		public Profile LoadProfile()
		{
			var profiles = LoadProfiles();
			foreach (var p in profiles) return p;
			return null;
		}
	}
}